...
export default function StudioPage({
  ...
  const activeSectionId = useSelector(
    (state: RootState) => state.ui.activeSectionId,
  );
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      if (res.success && res.version) {
        ...
        alert(
          `Published version ${res.version} (${res.bumpType} bump)\n\nChangelog: ${res.summary}`,
        );
      } else {
              ...
              <div className="space-y-4">
                  ...
                  <>
                    <div className="space-y-1">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          handlePropChange("quote", e.target.value)
                        }
                        disabled={role === "viewer"}
                      />
                    </div>
                    <div className="space-y-1">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          handlePropChange("author", e.target.value)
                        }
                        disabled={role === "viewer"}
                      />
                    </div>
                ...
                {activeSection?.type === "featureGrid" && (
                  <div className="space-y-1">
                    ...
                    <Input
                      value={(
                        (activeSection.props.features as string[]) || []
                      ).join(", ")}
                      onChange={(e) =>
                        handlePropChange(
                          "features",
